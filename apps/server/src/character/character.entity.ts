...
import { CharacterAchievements } from './embeddables/character-achievements.embeddable'
...
export class Character {
  ...
  @Embedded(() => CharacterAchievements, { nullable: true })
  achievements?: CharacterAchievements

  /**
   * @see https://develop.battle.net/documentation/world-of-warcraft/guides/character-renders
   */
  ...
  get avatar() {
    ...
    const gender = this.summary
      ? this.summary.gender === 'Male'
        ? 0
        : 1
  ...
  getStorage<C extends boolean | undefined>(
    endpoint: ProfileEndpoint,
    create: C
  ): C extends true ? EndpointStorage : EndpointStorage | undefined
  /**
  ...
  getStorage(endpoint: ProfileEndpoint, create?: boolean): EndpointStorage | undefined {
    switch (endpoint) {
      ...
      case 'character-achievements-summary':
        return (
          this.achievements ??
          (create ? (this.achievements = new CharacterAchievements()) : undefined)
        )
      ...
      case 'character-mythic-keystone-season':
        return (
          this.keystones ?? (create ? (this.keystones = new CharacterKeystoneProfile()) : undefined)
        )
