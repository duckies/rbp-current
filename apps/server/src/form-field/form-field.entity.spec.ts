...
import { CharacterFieldOptionsDTO, SelectFieldOptionsDTO } from './dto/create-field-options.dto'
...
describe('FormField (Entity)', () => {
  ...
  it('should validate character field input shape', () => {
    ...
    expect(field.isAnswerValid({ ...validAnswer, name: 'Duc kys' })).toBe(false)
    expect(field.isAnswerValid({ ...validAnswer, realm: 'area 52' })).toBe(false)
  ...
  it('should enforce main character option', () => {
    const field = new FormField()
    field.type = 'character'
    field.options = { requireMain: true } as CharacterFieldOptionsDTO
    ...
    expect(field.isAnswerValid(validAnswer)).toBe(true)
    expect(field.isAnswerValid({ ...validAnswer, main: undefined })).toBe(false)
  })

  it('should validate multi-character field input shape', () => {
    const field = new FormField()
    field.type = 'character'
    field.options = { multiple: true } as CharacterFieldOptionsDTO
    ...
    expect(field.isAnswerValid([characters[0], { ...characters[1], main: true }])).toBe(true)
    // We don't want affirmations in the negative.
    expect(field.isAnswerValid([characters[0], { ...characters[1], main: false }])).toBe(false)
    ;(field.options as CharacterFieldOptionsDTO).requireMain = true
